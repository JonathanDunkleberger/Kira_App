    ...
    <main className="min-h-screen bg-black text-white">
      <div className="container max-w-3xl mx-auto px-6 py-10 space-y-6">
        <div className="flex items-center justify-between">
          ...
          <Link href="/conversations" className="text-sm underline">Back</Link>
        </div>
        <div className="space-y-3">
          {messages.length === 0 && <div className="text-white/60">No messages yet.</div>}
          {messages.map((m: any, i: number) => (
            <div key={i} className={`p-3 rounded-lg border border-white/10 ${m.role === 'assistant' ? 'bg-fuchsia-500/10' : 'bg-white/5'}`}>
