...
export default function Sidebar() {
  ...
  return (
    <aside
      className={`hidden md:flex flex-col h-screen sticky top-0 bg-gray-900/50 border-r border-white/10 transition-all duration-300 ${isCollapsed ? 'w-16' : 'w-64'}`}
    >
      {/* Top: toggle + new chat */}
      ...
      {/* List */}
      <div
        className={`flex-1 overflow-y-auto p-2 ${isCollapsed ? '' : 'space-y-1'} custom-scrollbar scrollbar-hide scrollbar-hover`}
      >
        {!isCollapsed && (
          <>
            <h3 className="px-2 text-[11px] font-semibold uppercase tracking-wide text-white/50 mb-2">
              Recent
            </h3>
            {recent.length === 0 && <p className="text-xs text-white/50 px-2">No chats found.</p>}
            {recent.map((convo) => (
                  ...
                  >
                    ...
                    <DropdownMenu.Item
                      onSelect={async () => {
                        ...
                        if (!session) {
                          alert('Please sign in to rename.');
                          return;
                        }
            ...
            >
              ...
              {isPro && (
                <DropdownMenu.Item
                  ...
                  className="flex items-center gap-3 p-2 rounded hover:bg-neutral-800 cursor-pointer outline-none"
                >
