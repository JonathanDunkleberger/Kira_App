...
export default function LimitDialog({
  open, mode, remainingToday, remainingThisChat, onClose, onUpgrade, onNewChat
}: Props) {
  const isPaywall = mode === 'paywall';
  const capped = isPaywall ? (remainingToday === 0) : (remainingThisChat === 0);

  const title = isPaywall
    ? (capped ? 'Daily free limit reached' : 'You’re nearing today’s free limit')
    : (capped ? 'This chat hit its time limit' : 'This chat is nearing its limit');
  ...
  return (
    <Modal open={open} onClose={onClose} title={title} description={desc} footer={footer} />
  );
}
