...
export default function Live2DAvatar({ isSpeaking, analyserNode, emotion, accessories, action, onModelReady, onLoadError, onCanvasReady, isSceneActive }: Live2DAvatarProps) {
  ...
  const [zoomLevel, setZoomLevel] = useState(1.85);
  const zoomLevelRef = useRef(1.85);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      async function _initLive2D() {
        ...
        const yPosition = containerHeight * (isMobile ? 0.93 : 0.90);
        model.y = yPosition;
    ...
    const handleResize = () => {
      if (appRef.current?.renderer && containerRef.current) {
        ...
        if (modelRef.current) {
          ...
          baseYRef.current = h * (mobile ? 0.93 : 0.90);
  ...
  useEffect(() => {
    ...
    const MIN_ZOOM = 1.5;   // ~80% of default 1.85 — slight zoom out
    const MAX_ZOOM = 2.8;   // ~150% of default 1.85 — moderate zoom in
    const ZOOM_STEP = 0.1;
  ...
  return (
    <div style={{ width: "100%", height: "100%", maxWidth: isSceneActive ? "780px" : "600px", maxHeight: "90vh", margin: "0 auto", position: "relative", overflow: isSceneActive ? "visible" : "hidden", transition: "max-width 0.3s ease" }}>
      <div
        ref={containerRef}
        style={{
          width: "100%",
          height: "100%",
          pointerEvents: "auto",
          overflow: isSceneActive ? "visible" : "hidden",
