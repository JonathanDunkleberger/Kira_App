  ...
  const timer = setInterval(async () => {
    try {
      ...
      if (!paywall && !hardStop) {
        ...
        try {
          await svc.rpc('accrue_daily_usage', { p_user_id: userId, p_session_id: chatSessionId, p_inc: TICK });
        } catch (e) {
          // fallback to separate increments if atomic RPC missing
          try { await svc.rpc('increment_chat_elapsed', { p_chat_id: chatSessionId, p_seconds: TICK }); } catch {}
        }
      }
