...
import type { ClientEvent, ServerEvent } from './lib/voice-protocol.js';
...
function send(ws: WebSocket, obj: ServerEvent) {
  try {
...
async function finalizeUtterance(session: SessionState) {
  ...
  {
    let offset = 0;
    for (const buf of session.audioBuffers) {
  ...
  try {
    ...
    const body = JSON.stringify({
      ...
      messages: [
        {
          role: 'system',
          content: 'You are Kira, a concise, empathetic voice companion. Reply conversationally.',
        },
  ...
  if (assistantFull) {
    try {
      await prisma.message.create({
        data: { conversationId: session.conversationId, role: 'assistant', text: assistantFull },
      });
    } catch {}
  }
...
wss.on('connection', async (ws: WebSocket, req) => {
  ...
  ws.on('message', (raw: WebSocket.RawData, isBinary) => {
    ...
    if (typeof raw === 'string') {
      try {
        const msg: ClientEvent = JSON.parse(raw);
        switch (msg.t) {
          case 'client_ready':
            break;
          case 'eou':
